<script lang="ts">
  ...
  export let forceReveal: boolean = false;
    // For answer card layout
    $: nAnswers = question?.answers?.length || 0;
    $: perRow = nAnswers === 6 ? 3 : Math.ceil(nAnswers / 3);
      ...
      {#if nAnswers > 1}
        {#if nAnswers === 6}
          <div class="flex justify-center flex-wrap w-full mb-2">
            {#each question.answers.slice(0, 3) as ans, i}
              <AnswerCard answer={ans} questionId={question.id} forceReveal={forceReveal} number={i + 1} />
            {/each}
          </div>
          <div class="flex justify-center flex-wrap w-full">
            {#each question.answers.slice(3, 6) as ans, i}
              <AnswerCard answer={ans} questionId={question.id} forceReveal={forceReveal} number={i + 4} />
            {/each}
          </div>
        {:else}
          <div class="flex justify-center flex-wrap w-full mb-2">
            {#each question.answers.slice(0, perRow) as ans, i}
              <AnswerCard answer={ans} questionId={question.id} forceReveal={forceReveal} number={i + 1} />
            {/each}
          </div>
          <div class="flex justify-center flex-wrap w-full mb-2">
            {#each question.answers.slice(perRow, perRow * 2) as ans, i}
              <AnswerCard answer={ans} questionId={question.id} forceReveal={forceReveal} number={i + 1 + perRow} />
            {/each}
          </div>
          <div class="flex justify-center flex-wrap w-full">
            {#each question.answers.slice(perRow * 2) as ans, i}
              <AnswerCard answer={ans} questionId={question.id} forceReveal={forceReveal} number={i + 1 + perRow * 2} />
            {/each}
          </div>
        {/if}
    {:else}
